...
public class Invoice2csv {
	...
	public static void main(String[] args) {
		if (DEBUG)
			System.out.println("Invoice2csv.main("+args+")");
	...
	private static void parseAndSetArguments(String[] args) {
	    ...
	    if (args.length >= 5) {
	        ...
	        setDefaultSemanticBindingAndSkeleton();
	    }
	...
	private static void setDefaultSemanticBindingAndSkeleton() {
		if (0==PROCESSING.indexOf("JP-PINT")) {		
	...
	private static void processInvoice(String in_xml, String out_csv) {
		...
		for (Map.Entry<String, Binding> entry : FileHandler.bindingDict.entrySet()) {
			...
			if (id.matches("^([a-zA-Z0-9]+|[a-zA-Z0-9]+_([a-zA-Z0-9]_)?[a-zA-Z0-9]+)$") &&
					!id.matches("^.+_[0-9]+$") &&
					card.matches("^.*n$") &&
					isMultiple(sort)) {
		...
		if (DEBUG) System.out.println("-- END -- IN_XML "+in_xml);
	}
	...
	private static void fillJsonMeta(String out_csv, String out_json) throws IOException {
	    ...
	    for (String column : columns) {
	        ...
	        if (null==binding)
	        	System.out.println("bindingDict is not defined for "+id);
	    ...
	    try {
	        ...
	        if (DEBUG) System.out.println("JSON object written to " + out_json);
	    } catch (IOException e) {
	        e.printStackTrace();
	    }
	}
	...
	private static void fillTable() {
		...
		FileHandler.header.add("d_"+FileHandler.ROOT_ID);	
		// bough
		for (Map.Entry<Integer,String> multipleEntry : multipleMap.entrySet()) {
			...
			if (multipleID.matches("^([a-zA-Z0-9]+|[a-zA-Z0-9]+_([a-zA-Z0-9]_)?[a-zA-Z0-9]+)$") &&
					!multipleID.matches("^.+_[0-9]+$") &&
					multipleBinding.isUsed()) {
				FileHandler.header.add("d_"+multipleID);
			}
		}
		// data
		for (Map.Entry<Integer,Binding> dataEntry : FileHandler.semBindingMap.entrySet()) {
			...
			if (1!=dataSort &&
					dataID.matches("^([a-zA-Z0-9]+|[a-zA-Z0-9]+_([a-zA-Z0-9]_)?[a-zA-Z0-9]+)_[0-9]+$") &&
					dataBinding.isUsed() &&
		...
		for (Map.Entry<String, TreeMap<Integer, String>> entryRow : rowMapList.entrySet()) {
			...
			for (String bough : boughs) {
				...
				if (null==boughBinding) {
					if (DEBUG) System.out.println("fillTable boughBinding is null.");
				} else {
					String boughID  = boughBinding.getID();
					String boughSeq = index[1];
					if (boughSeq.equals("") || null==boughSeq) {
						boughSeq = "0";
					} else {
						int number = Integer.parseInt(boughSeq);
						number += 1;
						boughSeq = Integer.toString(number);
					}
					int boughIndex  = FileHandler.header.indexOf("d_"+boughID);
					if (boughIndex!=-1) {
		...
		// 2次元配列（FileHandler.tidyData）から、すべての行で値が定義されていない列を削除する.
        ...
        for (int y = 1; y < row_size; y++) {
            ArrayList<String> row = FileHandler.tidyData.get(y);
            if (row != null) {
                for (int x = 0; x < col_size; x++) {
                    String data = row.get(x);
        ...
        ArrayList<ArrayList<String>> revisedData = new ArrayList<>();
        for (int y = 0; y < row_size; y++) {
            ArrayList<String> row = new ArrayList<>();
            ArrayList<String> originalRow = FileHandler.tidyData.get(y);
            if (originalRow != null) {
                for (int x = 0; x < col_size; x++) {
                    if (usedList.get(x)) {
                        row.add(originalRow.get(x));
                    }
                }
            }
            revisedData.add(row);
        ...
        if (DEBUG) {
        	// Printing revised data for verification
	        for (ArrayList<String> row : revisedData) {
	            System.out.println(row);
	        }
        }

		// 2次元配列（FileHandler.tidyData）をすべての行で値が定義されていない列を削除したrevisedDataに置き換える.
		FileHandler.tidyData = revisedData;
		FileHandler.header   = revisedData.get(0);

		if (TRACE) System.out.println("* fillTable()  FileHandler.tidyData");
		for (int y = 0; y < row_size; y++) {
			ArrayList<String> row = new ArrayList<>();
	...
	private static void fillGroup (
		...
		// Query child elements using child XPath and returns child elements.
		...
		for (Integer childSort : childList.keySet()) { // childList includes both #text and @attribute
			...
			List<Node> children = childList.get(childSort); // Get retrieved child element.
			
			int countChildren = children.size();
			if (countChildren > 0) {
				if (countChildren > 1 && children.get(0).getNodeName().equals(children.get(1).getNodeName())) {
					if (childID.matches("^([a-zA-Z0-9]+|[a-zA-Z0-9]+_([a-zA-Z0-9]_)?[a-zA-Z0-9]+)_[0-9]+$")) {
						for (int i = 0; i < countChildren; i++) {
							
							fillMultipleBusinessTerm(boughMap, sort, childSort, children, i);
							
						}
					} else if (childID.matches("^[a-zA-Z0-9]+_([a-zA-Z0-9]_)?[a-zA-Z0-9]+$")) {
						for (int i = 0; i < countChildren; i++) {
				...
				for (int i = 0; i < countChildren; i++) {
					...
					String value         = child.getTextContent().trim(); // 下位要素のすべての文字データを取り出すので、ComplexTypeだと多くのデータが含まれている。
					if (PROCESSING.contains("JP-PINT") && !childNodeName.contains("Invoice") && !childNodeName.contains(":")) {
						
						fillData(childSort, value, boughMap); // @attribute
												
					} else if (null!=child && null!=value && value.length() > 0 &&
							childID.matches("^([a-zA-Z0-9]+|[a-zA-Z0-9]+_([a-zA-Z0-9]_)?[a-zA-Z0-9]+)_[0-9]+$")) {
						if (DEBUG) 
...
//			if (boughMap1.size() < 1+childLevel) {
...
//	}
	...
	private static void fillMultipleBusinessTermGroup(
		...
		if (DEBUG) 
			System.out.print("    boughMap lastKey="+lastkey+" child is multiple level="+childLevel);
		if (!childSort.equals(lastkey)) { // 2023-10-05
			if (boughMap1.size() < 1+childLevel) {
				boughMap1.put(childSort, i);
			} else {
				boughMap1.pollLastEntry();
	...
	private static void fillMultipleBusinessTerm(
		...
		TreeMap<Integer,Integer>
			boughMap1            = (TreeMap<Integer, Integer>) boughMap.clone();
		Integer lastkey          = boughMap1.lastKey();
		Integer lastvalue        = boughMap1.get(lastkey);
		if (DEBUG) 
			System.out.print("    boughMap lastKey="+lastkey+" child is multiple level="+childLevel);
		...
		if (DEBUG) 
			System.out.println("\n    UPDATED boughMapList="+boughMapList.toString()+
				"\n* fillData "+ businessTerm+" -> level="+childLevel+" "+childID+"("+childSort+") "+childBusinessTerm);
