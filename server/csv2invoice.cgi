...
sed 's/.*[[:blank:]]csvfile="\([^"]*\)".*/\1/'                           | # tee log/${0##*/}.$$.step5.log |
...
csvfile=${file_dirC}/${name}
cp $Tmp-uploadfile ${csvfile}
# === Generate Invoice ===============================================
basename=${name:0:-4}
xmlfile=${file_dirS}/${basename}.xml
(
  java -classpath /ebs/www/sambuichi.jp/public_html/core-japan/server/core-japan-0.0.1.jar wuwei.japan_core.cius.Csv2invoice ${$syntax} ${$csvfile} ${$xmlfile}
) &
# wait $!
# STATUS="${?}"
# === Return result ==================================================
...
{"csv":"${csvfile}","invoice":"${xmlfile}","syntax":"${syntax}","uuid":"${uuid}"}
HTML_CONTENT
rm -f $Tmp-*
exit 0
# csv2invoice.cgi