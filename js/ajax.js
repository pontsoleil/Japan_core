...
function AjaxSubmit (form) {
  var INDEX_URL = 'https://www.wuwei.space/core-japan/index.html'
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (form.method.toLowerCase() === "post") {
      ...
      for (nItem = 0; nItem < form.elements.length; nItem++) {
        ...
        if (fieldType === "FILE") {
          for (nFile = 0; nFile < field.files.length; nFile++) {
            var name = encodeURIComponent(field.name);
            var encodedname = encodeURIComponent(field.files[nFile++].name);
            sSearch += "&" + name + "=" + encodedname;
          }
        }
