<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <pattern id="UBL-modelaligned">
        <rule context="/ubl:Invoice/cac:AllowanceCharge[cbc:ChargeIndicator = false()] | /cn:CreditNote/cac:AllowanceCharge[cbc:ChargeIndicator = false()]">
            <assert id="aligned-ibrp-032-jp" flag="fatal" test="(exists(../cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']/cbc:ID = 'O') and not(exists(cac:TaxCategory[cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']/cbc:Percent))) or (exists(cac:TaxCategory[cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']/cbc:ID) and exists(cac:TaxCategory[cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']/cbc:Percent))">[aligned-ibrp-032jp]-Each Document level allowance (ibg-20) MUST have a Document level allowance tax category code (ibt-095) and Document level allowance tax rate (ibt-096), except if the allowance is outside the scope of tax.</assert>
        </rule>
        <rule context="/ubl:Invoice/cac:AllowanceCharge[cbc:ChargeIndicator = true()] | /cn:CreditNote/cac:AllowanceCharge[cbc:ChargeIndicator = true()]">
            <assert id="aligned-ibrp-037-jp" flag="fatal" test="(exists(../cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']/cbc:ID = 'O') and not(exists(cac:TaxCategory[cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']/cbc:Percent))) or (exists(cac:TaxCategory[cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']/cbc:ID) and exists(cac:TaxCategory[cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']/cbc:Percent))">[aligned-ibrp-037-jp]-Each Document level charge (ibg-21) MUST have a Document level charge tax category code (ibt-102) and Document level charge tax rate (ibt-103), except if the charge is outside the scope of tax.</assert>
        </rule>
        ...
        <rule context="/ubl:Invoice | /cn:CreditNote">
            ...
            <assert id="aligned-ibr-jp-04-ntr" flag="fatal" test="not(cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID)">[aligned-ibr-jp-04-ntr]-An Invoice must not  have the Seller tax identifier (ibt-031).</assert>
        ...
        <rule context="cac:TaxTotal">
            <assert id="aligned-ibr-jp-07-ntr" flag="fatal" test="xs:decimal(cbc:taxTotal) = 0">[aligned-ibr-jp-07-ntr] Invoice total TAX amount (ibt-110) MUST be 0 (zero).</assert>
        </rule>
        <rule context="cac:TaxTotal/cac:TaxSubTotal">
            <assert id="aligned-ibr-jp-08-ntr" flag="fatal" test="xs:decimal(cbc:taxTotal) = 0">[aligned-ibr-jp-08-ntr] TAX category tax amount (ibt-117) MUST be 0 (zero).</assert>
        </rule>
