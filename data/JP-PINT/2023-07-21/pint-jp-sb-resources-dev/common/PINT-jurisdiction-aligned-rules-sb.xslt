...
<xsl:template match="/ubl:Invoice | /cn:CreditNote" mode="M12" priority="1021">
    <svrl:fired-rule context="/ubl:Invoice | /cn:CreditNote" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="starts-with(normalize-space(cbc:CustomizationID/text()), 'urn:peppol:pint:selfbilling-1@jp-1')" />
      <xsl:otherwise>
        <svrl:failed-assert test="starts-with(normalize-space(cbc:CustomizationID/text()), 'urn:peppol:pint:selfbilling-1@jp-1')">
          <xsl:attribute name="id">aligned-ibrp-001-jp-sb</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>[aligned-ibrp-001-jp-sb]-Specification identifier MUST start with the value 'urn:peppol:pint:selfbilling-1@jp-1'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="/*/cbc:ProfileID and matches(normalize-space(/*/cbc:ProfileID), 'urn:peppol:bis:selfbilling')">
          ...
          <svrl:text>[aligned-ibrp-002-jp-sb]-Business process MUST be in the format 'urn:peppol:bis:selfbilling'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
